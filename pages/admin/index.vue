...
export default {
  ...
  computed: {
    ...mapGetters(["loadedRecipes", "authRecipes"])
  },
  methods: {
    filterBySearchText(text) {
      this.filteredRecipes = this.authRecipes.filter(
        recipe =>
          recipe.title.toLowerCase().includes(text.toLowerCase()) ||
          recipe.cuisine.toLowerCase().includes(text.toLowerCase()) ||
          recipe.ingredients.some(ingr =>
            ingr.toLowerCase().includes(text.toLowerCase())
          ) ||
          recipe.types.some(ingr =>
            ingr.toLowerCase().includes(text.toLowerCase())
          )
      );
    }
