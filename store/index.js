...
export const actions = {
  ...
  nuxtServerInit(vuexContext, context) {
    return axios
      .get("https://share-your-recipe.firebaseio.com/recipes.json")
      .then((res) => {
        let recipesArray = [];
  ...
  addRecipe(vuexContext, recipe) {
    ...
    return this.$axios
      .$post(
        `https://share-your-recipe.firebaseio.com/recipes.json?auth=${vuexContext.state.token}`,
        createdRecipe
      )
      .then((data) => {
  ...
  deleteRecipe(vuexContext, id) {
    ...
    return this.$axios
      .$delete(
        `https://share-your-recipe.firebaseio.com/recipes/${id}.json?auth=${vuexContext.state.token}`,
        { id: id }
      )
      .then((response) => {
        this.$router.go(0);
      ...
      this.$axios
        ...
        .catch((err) => {
          if (err.response.data.error.message === "EMAIL_EXISTS") {
            ...
            err.response.data.error.message === "MISSING_EMAIL"
          ) {
            vuexContext.commit("setEmailError", false);
            vuexContext.commit("setPasswordError", false);
            vuexContext.commit("setPasswordMissing", false);
            ...
            err.response.data.error.message === "MISSING_PASSWORD" ||
...
export const getters = {
  ...
  authRecipes: (state) =>
    state.loadedRecipes.filter((recipe) => recipe.email == state.email),
